...
public class ErrMsg {
	...
	public static String getTajikErrorMessage (String errorCode){
		
		int index = errorCode.indexOf( '/' );
		if(index != -1){
			
			String arrayMessage[] = StringUtil.split( errorCode , '/' );
			
			if(arrayMessage[0].equals( "SCREENING_FORM_NOT_FOUND" ))
				return (SCREENING_FORM_NOT_FOUND_1+arrayMessage[1]+SCREENING_FORM_NOT_FOUND_2);
			
			if(arrayMessage[0].equals("PATIENT_DETAILS_ALREADY_FILLED"))
			   return (PATIENT_DETAILS_ALREADY_FILLED_1+arrayMessage[1]+PATIENT_DETAILS_ALREADY_FILLED_2);
			
			if(arrayMessage[0].equals("PATIENT_DETAILS_NOT_FOUND"))
				return (PATIENT_DETAILS_NOT_FOUND_1+arrayMessage[1]+PATIENT_DETAILS_NOT_FOUND_2);
			
			if(arrayMessage[0].equals("BASELINE_DETAILS_ALEADY_FILLED"))
				return (BASELINE_DETAILS_ALEADY_FILLED_1+arrayMessage[1]+BASELINE_DETAILS_ALEADY_FILLED_2);
			
			if(arrayMessage[0].equals("BASELINE_DETAILS_NOT_FOUND"))
				return (BASELINE_DETAILS_NOT_FOUND_1+arrayMessage[1]+BASELINE_DETAILS_NOT_FOUND_2);
			
			if(arrayMessage[0].equals("TREATMENT_MONTH_ALREADY_FOUND"))
				return (TREATMENT_MONTH_ALREADY_FOUND_1+arrayMessage[1]+TREATMENT_MONTH_ALREADY_FOUND_2);
			
		}
		else{
			
			if (errorCode.equals( "ERROR_IN_SAVING_FORM" ))
				return (ERROR_IN_SAVING_FORM);
			
			if (errorCode.equals("DATE_ERROR_ENCOUNTER"))
				return (DATE_ERROR_ENCOUNTER);
			
			if (errorCode.equals("SAVE_ENCOUNTER_ERROR"))
				return (SAVE_ENCOUNTER_ERROR);
			
			if (errorCode.equals("SAVE_ENCOUNTER_RESULT_ERROR"))
				return (SAVE_ENCOUNTER_RESULT_ERROR);
